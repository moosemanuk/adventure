...
static OBJECT* getObject(const char* noun, OBJECT *from, DISTANCE maxDistance)
{
	...
	for (obj = objs; obj < endOfObjs; obj++)
	{
		if (objectHasTag(obj, noun) &&
			getDistance(from, obj) <= maxDistance)
		{
...
{
	OBJECT* obj = getObject(noun, player, dOverThere);
	if (obj == NULL)
	{
		if (getObject(noun, player, dNotHere) == NULL)
		{
...
OBJECT *getPossession(OBJECT* from, const char* verb, const char* noun)
{
	...
	else if ((obj = getObject(noun, from, dHeldContained)) == NULL)
	{
		if (getObject(noun, player, dNotHere))
		{
